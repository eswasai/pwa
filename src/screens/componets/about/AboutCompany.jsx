import VideoPopup from "@/modals/video-popup";
import React, { useState } from "react";

const company_content = {
  sub_title: "ABOUT FRAMESPIK",
  title: (
    <>
      Innovating Photography & <br />
      Secure Storage
    </>
  ),
  info_1: (
    <>
      Founded by a team of passionate professionals, Framespik combines
      photography expertise with advanced technology to deliver exceptional
      results. Under the leadership of CEO Lokesh Davuluri, we’ve expanded our
      offerings to include a comprehensive SaaS platform and secure cloud
      storage solutions.
    </>
  ),
  info_2: (
    <>
      Our SaaS platform offers a range of tools that streamline workflows, from
      event management and album creation to client collaboration. We empower
      photographers and event organizers with an intuitive, cloud-based solution
      that helps them organize, store, and share their work with ease.
    </>
  ),
  info_3: (
    <>
      Our cloud storage services ensure that your images and data are always
      safe, secure, and accessible.
    </>
  ),
};
const { sub_title, title, info_1, info_2, info_3 } = company_content;

const AboutCompany = () => {
  const [isVideoOpen, setIsVideoOpen] = useState(false);

  return (
    <>
      <div className="ab-company-area pt-105 pb-100">
        <div className="container">
          <div className="row ab-company-section-space">
            <div className="col-xl-6">
              <div className="ab-company-section-box">
                <h4 className="inner-section-subtitle">{sub_title}</h4>
                <h3 className="tp-section-title">{title}</h3>
              </div>
            </div>
            <div
              className="col-xl-6 wow tpfadeRight"
              data-wow-duration=".9s"
              data-wow-delay=".5s"
            >
              <div className="ab-company-right">
                <div className="ab-company-section-text">
                  <p className="pb-10">{info_1}</p>
                  <p className="pb-10">{info_2}</p>
                  <p>
                    <span>{info_3}</span>
                  </p>
                </div>
              </div>
            </div>
          </div>
          <div className="row align-items-center">
            <div className="col-xl-4">
              <div className="ab-company-video">
                <a className="popup-video" onClick={() => setIsVideoOpen(true)}>
                  <i className="fas fa-play"></i>
                </a>
                <span>Software tour</span>
              </div>
            </div>
            <div className="col-xl-8">
              <div className="row">
                <div className="col-md-4 col-sm-4 mb-40">
                  <div className="ab-company-fun-fact-wrap d-flex justify-content-start">
                    <div className="ab-company-fun-fact">
                      <span>Trusted By</span>
                      <h4>
                        1T<em>+</em>
                      </h4>
                      <p>Belived Customers</p>
                    </div>
                  </div>
                </div>

                <div className="col-md-4 col-sm-4 mb-40">
                  <div className="ab-company-fun-fact-wrap d-flex justify-content-md-center justify-content-left">
                    <div className="ab-company-fun-fact">
                      <span>Storage</span>
                      <h4>
                        100<em>TB</em>
                      </h4>
                      <p>Maintained</p>
                    </div>
                  </div>
                </div>
                <div className="col-md-4 col-sm-4 mb-40">
                  <div className="ab-company-fun-fact-wrap ab-company-border-none d-flex justify-content-md-center justify-content-left">
                    <div className="ab-company-fun-fact">
                      <span>IN</span>
                      <h4>
                        500<em>+</em>
                      </h4>
                      <p>Cities using</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      {/* video modal start */}
      <VideoPopup
        isVideoOpen={isVideoOpen}
        setIsVideoOpen={setIsVideoOpen}
        videoId={"HsRtw4Jrluo"}
      />
      {/* video modal end */}
    </>
  );
};

export default AboutCompany;
